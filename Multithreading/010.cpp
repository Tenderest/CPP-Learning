...
namespace ff {
    ...
    class Callable {
    public:
        int my_thread_005_in_class(int par)
        {
            std::cout << "par: " << par << std::endl;
            std::cout << "-- my thread start ==> " << std::this_thread::get_id() << std::endl;
            // std::cout << "-- my_thread id is: " << std::this_thread::get_id() << std::endl;
...
int main(void)
{
    ...
    // std::future<int> ret_thread_005_par = std::async(ff::my_thread_005, tmp_par);

    // std::future<int> ret_thread_005_in_class = std::async(&ff::Callable::my_thread_005_in_class, &callable, tmp_par);
    std::future<int> ret_thread_005_in_class = std::async(std::launch::deferred, &ff::Callable::my_thread_005_in_class, &callable, tmp_par);
    ...
    int m_i;
    m_i = 0;
    std::cout << ret_thread_005_in_class.get() << std::endl; // 只有等待线程完毕才能获取结果
